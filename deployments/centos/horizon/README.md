...
* ```身份認證服務```的主機使用者可以利用 sudo 權限。因為 httpd 不提供從 Root 使用者來取得相關內容，管理者若要執行 Horizon  的話，必須讓 Keystone 擁有 ```sudo``` 使用權。
...
* 開啟 Web 瀏覽器進入儀表板: http://controller/dashboard。
