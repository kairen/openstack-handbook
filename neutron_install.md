# Neutron 安裝與設定
> 這邊若```NOVA_DBPASS```要更改的話，可以更改。# Neutron install與設定
本章節會說明與操作如何安裝```Neutron```服務到OpenStack Controller節點上，並設置相關參數與設定。若對於Neutron不瞭解的人，可以參考[Neutron 網路套件章節](neutron.html)
